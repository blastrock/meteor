...
#define W32(add, val) *(uint32_t*)(m_iomem + ((add)&0xFFF)) = (val)
...
{
  // debug ("IO Read8 at " << IOS_ADD << add << " of " << IOS_ADD <<
  // (int)*(uint8_t*)(m_iomem + (add & 0xFFF)));
  if ((add & 0xFF0) == 0x100)
    switch (add & 0xF)
    {
    case 0x0:
...
{
  // debug ("IO Read16 at " << IOS_ADD << add << " of " << IOS_ADD <<
  // *(uint16_t*)(m_iomem + (add & 0xFFF)));
  // special case, reading timers
  if ((add & 0xFF0) == 0x100)
    switch (add & 0xF)
    {
    case 0x0:
      return TIMER0.GetCount();
    case 0x4:
      return TIMER1.GetCount();
    case 0x8:
      return TIMER2.GetCount();
    case 0xC:
      return TIMER3.GetCount();
    }
...
{
  // debug ("IO Read32 at " << IOS_ADD << add << " of " << IOS_ADD <<
  // *(uint32_t*)(m_iomem + (add & 0xFFF)));
  // special case, reading timers
  if ((add & 0xFF0) == 0x100)
    switch (add & 0xF)
    {
