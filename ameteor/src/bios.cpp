...
void Bios::LZ77UnCompWram()
{
  ...
  debug(
      "LZ77UnCompWram from " << IOS_ADD << R(0) << " to " << IOS_ADD << R(1)
                             << ", len : " << size);
  uint32_t dest = R(1);
  uint8_t flags;
  uint16_t block;
...
void Bios::LZ77UnCompVram()
{
  ...
  uint32_t size = header >> 8;
  debug(
      "LZ77UnCompVram from " << IOS_ADD << R(0) << " to " << IOS_ADD << R(1)
                             << ", len : " << size);
  uint32_t dest = R(1);
  uint8_t flags;
  uint16_t out = 0;
...
void Bios::RLUnCompWram()
{
  ...
  uint32_t size = header >> 8;
  debug(
      "RLUnCompWram from " << IOS_ADD << R(0) << " to " << IOS_ADD << R(1)
                           << ", len : " << size);
  uint32_t dest = R(1);
  uint8_t flags;
  uint8_t block;
...
void Bios::RLUnCompVram()
{
  ...
  uint32_t size = header >> 8;
  debug(
      "RLUnCompVram from " << IOS_ADD << R(0) << " to " << IOS_ADD << R(1)
                           << ", len : " << size);
  uint32_t dest = R(1);
  uint8_t flags;
  uint16_t out = 0;
