...
ARM(BXBLX)
{
  if ((code & 0x0FFFFF00) != 0x012FFF00)
    met_abort(
        "Bits 8-27 must be 0001 00101111 11111111 for BX/BLX instructions");
...
ARM(_DataProcShiftReg)
{
  ...
  if (code & (0x1 << 7))
    met_abort(
        "Bit 7 must be 0 for DataProc with shift by register instructions");
    ...
    uint8_t rd, uint32_t op1, uint32_t op2, bool shiftcarry)
{
  ...
  if (opcode < 0x8 || opcode > 0xB)
  {
    ...
    met_abort(
        "Set condition bit not set for test operation or Rd not acceptable for "
        "a test");
...
ARM(PSR)
{
  ...
  if (code & (0x1 << 21)) // MSR
  {
    ...
    else if (oncpsr && (psr & 0x1F) != (val & 0x1F)) // have we changed mode ?
...
ARM(LDRSTR)
{
  ...
  if (code & (0x1 << 25)) // register offset
  {
    if (code & (0x1 << 4))
      met_abort(
          "Bit 4 must be 0 for LDR or STR instruction with register offset");
    offset = (code >> 7) & 0x1F;
    switch ((code >> 5) & 0x3)
    {
    case 0: // Logical Shift Left
...
ARM(STRLDR_HD)
{
  ...
  else // register offset
  {
    if ((code >> 8) & 0xF)
      met_abort(
          "Bits 8-11 must be 0 for halfword transfer with register offset "
          "instructions");
    NOT_PC(Rm);
    off = R(Rm);
  }
  ...
  if (code & (0x1 << 24))
  {
    ...
    met_abort(
        "Bit 21 must be 0 for post indexed halfword transfers instructions");
...
ARM(LDMSTM)
{
  ...
  uint8_t numregs = NumBits[(code >> 12) & 0xF] + NumBits[(code >> 8) & 0xF] +
                    NumBits[(code >> 4) & 0xF] + NumBits[code & 0xF];
...
ARM(_Code)
{
  if (!a_CheckCondition(code >> 28)) // condition failed
    ...
    default:
    {
      std::cerr << IOS_ADD << R(15) - 8 << " : " << IOS_ADD << code << " : ";
      debug_bits(code);
      met_abort("not implemented");
    }
    break;
