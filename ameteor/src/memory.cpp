...
#else
uint8_t* Memory::GetRealAddress(
    uint32_t add, uint8_t size __attribute__((unused)))
{
...
#ifdef NO_MEMMEM // memmem() is a GNU extension, and does not exist in at least
                 // MinGW.
#define memmem memmem_compat
// Implementation from Git.
static void* memmem_compat(
    const void* haystack,
    size_t haystack_len,
    const void* needle,
    size_t needle_len)
