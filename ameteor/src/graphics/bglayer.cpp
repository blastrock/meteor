...
void BgLayer::DrawLine0(uint8_t line, uint16_t* ptr)
{
  ...
  if (yoff >= 256)
    if (m_tWidth == 64)
      if (xoff >= 256)
        // zone 4
        pTile = pMap + 32 * 32 + 32 * 32 + 32 * 32 + 32 * ((yoff - 256) / 8) +
                (xoff - 256) / 8;
      else
        // zone 3 (large map)
...
void BgLayer::DrawLine2(
    uint16_t* ptr, int32_t curX, int32_t curY, int16_t dx, int16_t dy)
{
  if (!m_hicolor)
  {
  ...
  for (uint8_t x = 0; x < 240; ++x, ++ptr, curX += dx, curY += dy)
  {
    ...
    colorInd = pChar
        [pMap[intY / 8 * m_rWidth + intX / 8] * 8 * 8 + (intY % 8) * 8 +
         intX % 8];
    ...
    bool frame1)
{
  ...
  uint8_t* pChar = m_memory.GetRealAddress(frame1 ? 0x0600A000 : 0x06000000);
