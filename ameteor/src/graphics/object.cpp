...
void Object::DrawLine(
    uint8_t line, uint32_t* surface, bool oneDim, uint8_t mosaic)
{
    ...
    uint8_t mosaic)
{
  ...
  int32_t curX = ((m_width * 8) << 8) / 2 + (-fwidth / 2) * a +
                 (line - yoff - fheight / 2) * b;
  int32_t curY = ((m_height * 8) << 8) / 2 + (-fwidth / 2) * c +
                 (line - yoff - fheight / 2) * d;
  int32_t intX, intY;
  ...
  if (m_attr0 & (0x1 << 13)) // if 256 colors mode
  {
    for (uint8_t i = 0; i < fwidth; ++i)
    {
      ...
      if (intX >= 0 && intX < m_width * 8 && intY >= 0 && intY < m_height * 8 &&
...
void Object::DrawWindow(
    uint8_t line, uint8_t* surface, bool oneDim, uint8_t mask)
{
    ...
    uint8_t mask)
{
  ...
  if (m_attr0 & (0x1 << 13)) // if 256 colors mode
  {
    for (uint8_t i = 0; i < fwidth; ++i)
    {
      ...
      {
        colorInd = pChar
            [(intY / 8) * (oneDim ? m_width : 32) * 8 * 8 + (intX / 8) * 8 * 8 +
             (intY % 8) * 8 + (intX % 8)];
...
void Object::UpdateAttrs(uint16_t attr0, uint16_t attr1, uint16_t attr2)
{
  ...
  if (setsize)
  {
    ...
    if (reload)
    {
      ...
      m_charEnd = m_charBegin +
                  m_width * m_height * 8 * ((m_attr0 & (0x1 << 13)) ? 8 : 4);
    }
  }
}
