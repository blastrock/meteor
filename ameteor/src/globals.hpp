...
#define CYCLES16Seq(add, count) CLOCK.TimePass(MEM.GetCycles16Seq(add, count))
#define CYCLES32NSeq(add, count) \
  CLOCK.TimePass(MEM.GetCycles32NoSeq(add, count))
#define CYCLES32Seq(add, count) CLOCK.TimePass(MEM.GetCycles32Seq(add, count))
#define ICYCLES(i) CLOCK.TimePass(i)
...
#define SETF(flag, val) FLAG_##flag = (val) ? 1 : 0
#define SETFB(flag, val) FLAG_##flag = (val)

#define FZ(val) SETF(Z, !(val))
#define FN(val) SETF(N, NEG(val))
...
#define ADDCARRY(a, b, c) \
  ((NEG(a) & NEG(b)) | (NEG(a) & POS(c)) | (NEG(b) & POS(c)))
#define ADDOVERFLOW(a, b, c) \
  ((NEG(a) & NEG(b) & POS(c)) | (POS(a) & POS(b) & NEG(c)))
#define SUBCARRY(a, b, c) \
  ((NEG(a) & POS(b)) | (NEG(a) & POS(c)) | (POS(b) & POS(c)))
#define SUBOVERFLOW(a, b, c) \
  ((NEG(a) & POS(b) & POS(c)) | (POS(a) & NEG(b) & NEG(c)))
...
#define MET_UNUSED(v) __attribute__((unused)) MET_UNUSED_##v
