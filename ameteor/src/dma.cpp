...
void Dma::UpdateCnt(uint8_t channum)
{
  ...
  if (chan.control.w != IO.DRead16(Io::DMA0CNT_H + channum * Io::DMA_CHANSIZE))
  {
    if (!chan.control.b.enable &&
        (IO.DRead16(Io::DMA0CNT_H + channum * Io::DMA_CHANSIZE) & (0x1 << 15)))
    // if we changed enable from 0 to 1
    {
      ...
      chan.control.w = IO.DRead16(Io::DMA0CNT_H + channum * Io::DMA_CHANSIZE);
    ...
    if (chan.control.b.start == Special)
    {
      ...
      case 2:
        ...
        if (chan.dest != 0x040000A0 && chan.dest != 0x040000A4)
          met_abort(
              "Special DMA 1 or 2 with unauthorized address : " << IOS_ADD
                                                                << chan.dest);
...
void Dma::Check(uint8_t channum, uint8_t reason)
{
  ...
  if (cnt.b.enable && cnt.b.start == reason)
    Process(channum);
}
...
void Dma::Process(uint8_t channum)
{
  ...
  // chan.dest, chan.count, chan.control.b.type ? "words":"halfwords"); if (i >
  // debut)
  debug(
      "DMA" << IOS_NOR << (int)channum << ", from " << IOS_ADD << chan.src
            << " to " << IOS_ADD << chan.dest << " of " << IOS_NOR
            << (chan.count ? chan.count : 0x10000)
